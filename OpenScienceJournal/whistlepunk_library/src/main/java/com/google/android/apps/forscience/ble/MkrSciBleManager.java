...
public class MkrSciBleManager {
    ...
    private static class GattHandler extends BluetoothGattCallback {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            BluetoothGattService service = this.gatt.getService(UUID.fromString(SCIENCE_KIT_SERVICE_UUID));
            if (service != null) {
                characteristics.addAll(service.getCharacteristics());
                BluetoothGattCharacteristic c = getCharacteristic(SCIENCE_KIT_VERSION_UUID);
                if (c != null) {
                    this.gatt.readCharacteristic(c);
                }
                return;
            }
            service = this.gatt.getService(UUID.fromString(NANO_33_BLE_SENSE_SERVICE_UUID));
            if (service != null) {
                characteristics.addAll(service.getCharacteristics());
                BluetoothGattCharacteristic c = getCharacteristic(NANO_33_BLE_SENSE_VERSION_UUID);
                if (c != null) {
                    this.gatt.readCharacteristic(c);
                }
            }
        ...
        public void onCharacteristicRead(
            ...
            if (firmwareVersion == -1 && (SCIENCE_KIT_VERSION_UUID.equals(uuid) || NANO_33_BLE_SENSE_VERSION_UUID.equals(uuid))) {
        ...
        public void onCharacteristicChanged(
            ...
            switch (uuid) {
                case SCIENCE_KIT_INPUT_1_UUID:
                case SCIENCE_KIT_INPUT_2_UUID:
                case SCIENCE_KIT_INPUT_3_UUID:
                case NANO_33_BLE_SENSE_PROXIMITY_UUID:
                    type = ValueType.UINT16;
                    break;
                case SCIENCE_KIT_VOLTAGE_UUID:
                case SCIENCE_KIT_CURRENT_UUID:
                case SCIENCE_KIT_RESISTANCE_UUID:
                ...
                case SCIENCE_KIT_GYROSCOPE_UUID:
                case SCIENCE_KIT_MAGNETOMETER_UUID:
                case NANO_33_BLE_SENSE_ACCELEROMETER_UUID:
                case NANO_33_BLE_SENSE_GYROSCOPE_UUID:
                case NANO_33_BLE_SENSE_MAGNETOMETER_UUID:
                    type = ValueType.SFLOAT_ARR;
                    break;
                case NANO_33_BLE_SENSE_COLOR_UUID:
                    type = ValueType.INT32_ARR;
                    break;
                default:
                    type = null;
            }
